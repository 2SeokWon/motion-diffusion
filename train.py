...
def train():
    ...
    position_features = 3
    rotation_features = 6
    root_features = 4 # 루트 Y높이(1) + delta(3)
    foot_features = 2
    cond_features = 3
    joint_position_features = (njoints - 1) * position_features #66
    joint_rotation_features = njoints * rotation_features #138

    input_feats = root_features + joint_position_features + joint_rotation_features + foot_features + cond_features  #213
    seq_len = 180
    num_timesteps = 1000
    ...
    processed_data_path = "./processed_data_interp30_1226/"
    ...
    for epoch in range(num_epochs):
        ...
        for batch in progress_bar:
            ...
            cond = x_start[:, :, 210:213] #interpolated traj
            ...
            progress_bar.set_postfix({
                ...
                'cond': f'{total_cond / (progress_bar.n + 1):.4f}',
        ...
        print(f"Epoch [{epoch+1}/{num_epochs}], Loss: {avg_loss:.4f}, Root: {avg_root:.4f}, Joint: {avg_joint:.4f}, Foot: {avg_foot:.4f}, Cond: {avg_cond:.4f}")
        wandb.log({
            "epoch": epoch + 1,
            "avg_loss": avg_loss,
            "avg_root": avg_root,
