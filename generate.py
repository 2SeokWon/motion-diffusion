...
def generate():
    ...
    parser.add_argument('--vel_path', type=str, default='./traj_from_template/velocity_compatible.pt')
    parser.add_argument('--abs_traj_path', type=str, default='./traj_from_template/absolute_3d_compatible.pt')
    parser.add_argument('--processed_data_path', type=str, default='./processed_data_position_1125')
    ...
    pos_vel_mean, pos_vel_std = dataset.root_pos_mean, dataset.root_pos_std #4
    ...
    displace_mean, displace_std = dataset.displacement_mean, dataset.displacement_std #3
    
    full_mean = np.hstack([pos_vel_mean, position_mean, rotation_mean, foot_mean, displace_mean])  # 213
    full_std  = np.hstack([pos_vel_std,  position_std,  rotation_std,  foot_std, displace_std])   # 213

    vel_pt = torch.load(args.vel_path, map_location='cpu').numpy().astype(np.float32)  # [T,4]
    abs_pt = torch.load(args.abs_traj_path, map_location='cpu').numpy().astype(np.float32)  # [T,3]
    
    cond_vel_norm = (vel_pt - pos_vel_mean) / pos_vel_std
    cond_traj_norm = (abs_pt - displace_mean) / displace_std
    cond_norm = torch.from_numpy(np.hstack([cond_vel_norm, cond_traj_norm])).float().unsqueeze(0).to(device)  # [1, T, 7]
    ...
    gen_traj_abs =  generated_motion[:, 210:213].copy()  # [T,3] #루트의 절대 위치 (vx,vz,yaw_rate)
